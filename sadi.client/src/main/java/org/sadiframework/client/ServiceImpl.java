...
public class ServiceImpl extends ServiceBase
{
	...
	void loadServiceModel() throws SADIException
	{
		...
		try {
			model.read(getURI());
			log.debug("INIT LOAD SERVICE MODEL" );
	...
	{
		System.out.println("### invokeService - ServiceImpl.java ###");
		OntClass inputClass;
		try {
		...
		System.out.println(String.format("assembling input to %s", this));
		while (inputNodes.hasNext()) {
			Resource inputNode = inputNodes.next();
			System.out.println(String.format("computing minimal RDF for %s as an instance of %s", inputNode, inputClass));
	...
	{
		System.out.println("### invokeServiceUnparsed - ServiceImpl.java ###");
		if (log.isTraceEnabled()) {
			log.trace(String.format("posting 1 RDF to %s:\n%s", getURI(), RdfUtils.logModel(inputModel)));
		}

		Model model = ModelFactory.createDefaultModel();
		try {
			System.out.println("get URI = "+ getURI().toString());
			InputStream is = HttpUtils.postToURL(new URL(getURI()), inputModel);	
			System.out.println("get URI2111 = "+ getURI().toString());

			
			//String theString = IOUtils.toString(is); 
			//System.out.println("INOUT STREAAMM = "+ theString); //PREMATURE END OF FILE
			System.out.println("get URI222 = "+ getURI().toString());

			model.read(is, "");
			model.write(System.out, "RDF/XML");
			System.out.println("get URI3 = "+ getURI().toString());
			is.close();
		} catch (IOException e) {
			throw new ServiceInvocationException(String.format("111-error communicating with service: %s", e.getMessage()), e);
		} catch (Exception e) {
			throw new ServiceInvocationException(String.format("error parsing service response: %s", e.getMessage()), e);
		}
		
		System.out.println("#@-#!!!### invoke serviceUnparsed Service Impl - inputModel=");
		//inputModel.write(System.out,"RDF/XML");		
		//System.out.println("#@-#!!!### invoke serviceUnparsed Service Impl - Model=");
		//model.write(System.out,"RDF/XML");		
